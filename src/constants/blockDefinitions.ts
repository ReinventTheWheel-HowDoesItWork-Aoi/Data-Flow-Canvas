...
`,
  ...
  'datetime-extract': {
    ...
    description: 'Break dates into year, month, day, weekday',
    icon: 'Calendar',
    defaultConfig: {
      column: '',
      extractYear: true,
...
if not column:
    raise ValueError("Date/Time Extract: Please specify a column to extract from in the Config tab")
...
try:
    ...
    raise ValueError(f"Date/Time Extract: Could not convert '{column}' to datetime: {str(e)}")

prefix = config.get('prefix', '')
if prefix:
    prefix = prefix + '_'

if config.get('extractYear', True):
    df[f'{prefix}year'] = df[column].dt.year
if config.get('extractMonth', True):
    df[f'{prefix}month'] = df[column].dt.month
if config.get('extractDay', True):
    df[f'{prefix}day'] = df[column].dt.day
if config.get('extractWeekday', True):
    df[f'{prefix}weekday'] = df[column].dt.weekday
if config.get('extractHour', False):
    df[f'{prefix}hour'] = df[column].dt.hour
if config.get('extractMinute', False):
    df[f'{prefix}minute'] = df[column].dt.minute
...
`,
  ...
  'string-operations': {
    ...
    description: 'Clean and manipulate text',
    icon: 'Type',
    defaultConfig: {
      column: '',
      operation: 'lower',
...
if operation == 'lower':
    df[output_col] = df[column].astype(str).str.lower()
elif operation == 'upper':
    df[output_col] = df[column].astype(str).str.upper()
elif operation == 'title':
    df[output_col] = df[column].astype(str).str.title()
elif operation == 'strip':
    df[output_col] = df[column].astype(str).str.strip()
elif operation == 'replace':
    if not pattern:
        raise ValueError("String Operations: Please specify a pattern to replace")
    df[output_col] = df[column].astype(str).str.replace(pattern, replacement, regex=False)
elif operation == 'replace_regex':
    if not pattern:
        raise ValueError("String Operations: Please specify a regex pattern to replace")
    df[output_col] = df[column].astype(str).str.replace(pattern, replacement, regex=True)
elif operation == 'extract':
    if not pattern:
        raise ValueError("String Operations: Please specify a regex pattern to extract")
    df[output_col] = df[column].astype(str).str.extract(f'({pattern})', expand=False)
elif operation == 'len':
    df[output_col] = df[column].astype(str).str.len()
elif operation == 'split_first':
    if not pattern:
        raise ValueError("String Operations: Please specify a delimiter to split on")
    df[output_col] = df[column].astype(str).str.split(pattern).str[0]
elif operation == 'split_last':
    if not pattern:
        raise ValueError("String Operations: Please specify a delimiter to split on")
    df[output_col] = df[column].astype(str).str.split(pattern).str[-1]
...
`,
  ...
  'window-functions': {
    ...
    description: 'Rolling averages, running totals, lag/lead',
    icon: 'TrendingUp',
    defaultConfig: {
      column: '',
...
def apply_operation(series):
    if operation == 'rolling_mean':
        ...
        return series.shift(window_size)
    elif operation == 'shift_back':
        return series.shift(-window_size)
    elif operation == 'diff':
        return series.diff(window_size)
    elif operation == 'pct_change':
        return series.pct_change(window_size)
    else:
        raise ValueError(f"Unknown operation: {operation}")

if group_by:
    df[new_column] = df.groupby(group_by)[column].transform(apply_operation)
else:
    df[new_column] = apply_operation(df[column])
...
if labels_str:
    ...
    if len(labels) != bins:
        raise ValueError(f"Bin/Bucket: Number of labels ({len(labels)}) must match number of bins ({bins})")

# Convert column to numeric
try:
    numeric_col = pd.to_numeric(df[column], errors='coerce')
except Exception as e:
    raise ValueError(f"Bin/Bucket: Could not convert '{column}' to numeric: {str(e)}")

if method == 'equal_width':
    df[output_col] = pd.cut(numeric_col, bins=bins, labels=labels)
elif method == 'equal_freq':
    df[output_col] = pd.qcut(numeric_col, q=bins, labels=labels, duplicates='drop')
elif method == 'custom':
    # For custom, bins should be a comma-separated list of edges
    # e.g., "0,10,20,50,100"
    raise ValueError("Bin/Bucket: For custom bins, use the expression: pd.cut(df['col'], bins=[0,10,20,50,100])")
...
`,
  ...
  'rank': {
    ...
    category: 'transform',
    label: 'Rank',
    description: 'Assign position (1st, 2nd, 3rd...)',
    icon: 'Trophy',
    defaultConfig: {
      column: '',
      method: 'average',
...
if not column:
    raise ValueError("Rank: Please specify a column to rank in the Config tab")
...
if group_by:
    ...
    df[output_col] = df[column].rank(method=method, ascending=ascending)
...
`,
  ...
  'type-conversion': {
    ...
    description: 'Change text to number to date',
    icon: 'RefreshCw',
    defaultConfig: {
      column: '',
      targetType: 'numeric',
...
df = input_data.copy()
column = config.get('column', '')
target_type = config.get('targetType', 'numeric')
...
try:
    if target_type == 'numeric':
        ...
        df[column] = pd.to_numeric(
            df[column].astype(str).str.strip().str.replace(',', ''),
            errors=errors
        )
        ...
        raise ValueError(f"Type Conversion: Unknown target type '{target_type}'")
except Exception as e:
    raise ValueError(f"Type Conversion: Error converting '{column}' to {target_type}: {str(e)}")
